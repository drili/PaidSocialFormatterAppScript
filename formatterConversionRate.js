function formatterConversionRate() {
    ...
    // *** current30D [START]
    var range = currentSheet.getRange("CC11:CC100");
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuePrev30D = currentSheet.getRange("CB" + (i + 11)).getValue();

        data["current30D"] = row[0];
        data["prev30D"] = valuePrev30D;
        data["field_index_value"] = "CC" + (i + 11);
    ...
    // *** current7D [START]
    var range = currentSheet.getRange("CE11:CE100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valueAvg7D = currentSheet.getRange("CF" + (i + 11)).getValue();

        data["current7D"] = row[0];
        data["avg7D"] = valueAvg7D;
        data["field_index_value"] = "CE" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
    ...
    // *** yesterday [START]
    var range = currentSheet.getRange("CH11:CH100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuelastWeek = currentSheet.getRange("CG" + (i + 11)).getValue();

        data["yesterday"] = row[0];
        data["lastWeek"] = valuelastWeek;
        data["field_index_value"] = "CH" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
