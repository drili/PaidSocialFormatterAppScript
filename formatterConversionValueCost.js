function formatterConversionValueCost() {
    ...
    // *** current30D [START]
    var range = currentSheet.getRange("DM11:DM100");
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuePrev30D = currentSheet.getRange("DL" + (i + 11)).getValue();

        data["current30D"] = row[0];
        data["prev30D"] = valuePrev30D;
        data["field_index_value"] = "DM" + (i + 11);
    ...
    // *** current7D [START]
    var range = currentSheet.getRange("DO11:DO100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valueAvg7D = currentSheet.getRange("DP" + (i + 11)).getValue();

        data["current7D"] = row[0];
        data["avg7D"] = valueAvg7D;
        data["field_index_value"] = "DO" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
    ...
    // *** yesterday [START]
    var range = currentSheet.getRange("DR11:DR100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuelastWeek = currentSheet.getRange("DQ" + (i + 11)).getValue();

        data["yesterday"] = row[0];
        data["lastWeek"] = valuelastWeek;
        data["field_index_value"] = "DR" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
