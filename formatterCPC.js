function formatterCPC() {
    ...
    // *** current30D [START]
    var range = currentSheet.getRange("AS11:AS100");
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuePrev30D = currentSheet.getRange("AR" + (i + 11)).getValue();

        data["current30D"] = row[0];
        data["prev30D"] = valuePrev30D;
        data["field_index_value"] = "AS" + (i + 11);
    ...
    // *** current7D [START]
    var range = currentSheet.getRange("AU11:AU100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valueAvg7D = currentSheet.getRange("AV" + (i + 11)).getValue();

        data["current7D"] = row[0];
        data["avg7D"] = valueAvg7D;
        data["field_index_value"] = "AU" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
    ...
    // *** yesterday [START]
    var range = currentSheet.getRange("AX11:AX100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuelastWeek = currentSheet.getRange("AW" + (i + 11)).getValue();

        data["yesterday"] = row[0];
        data["lastWeek"] = valuelastWeek;
        data["field_index_value"] = "AX" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
