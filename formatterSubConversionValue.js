function formatterSubConversionValue() {
    ...
    // *** current30D [START]
    var range = currentSheet.getRange("CU11:CU100");
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuePrev30D = currentSheet.getRange("CT" + (i + 11)).getValue();

        data["current30D"] = row[0];
        data["prev30D"] = valuePrev30D;
        data["field_index_value"] = "CU" + (i + 11);
    ...
    // *** current7D [START]
    var range = currentSheet.getRange("CW11:CW100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valueAvg7D = currentSheet.getRange("CX" + (i + 11)).getValue();

        data["current7D"] = row[0];
        data["avg7D"] = valueAvg7D;
        data["field_index_value"] = "CW" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
    ...
    // *** yesterday [START]
    var range = currentSheet.getRange("CZ11:CZ100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuelastWeek = currentSheet.getRange("CY" + (i + 11)).getValue();

        data["yesterday"] = row[0];
        data["lastWeek"] = valuelastWeek;
        data["field_index_value"] = "CZ" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
