function formatterConversionValue() {
    ...
    // *** current30D [START]
    var range = currentSheet.getRange("DD11:DD100");
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuePrev30D = currentSheet.getRange("DC" + (i + 11)).getValue();

        data["current30D"] = row[0];
        data["prev30D"] = valuePrev30D;
        data["field_index_value"] = "DD" + (i + 11);
    ...
    // *** current7D [START]
    var range = currentSheet.getRange("DF11:DF100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valueAvg7D = currentSheet.getRange("DG" + (i + 11)).getValue();

        data["current7D"] = row[0];
        data["avg7D"] = valueAvg7D;
        data["field_index_value"] = "DF" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
    ...
    // *** yesterday [START]
    var range = currentSheet.getRange("DI11:DI100");
    var values = range.getValues();
    ...
    for (var i = 0; i < values.length; i++) {
        var row = values[i];
        var data = {};
        var valuelastWeek = currentSheet.getRange("DH" + (i + 11)).getValue();

        data["yesterday"] = row[0];
        data["lastWeek"] = valuelastWeek;
        data["field_index_value"] = "DI" + (i + 11);
        dataArray.push({ ["data_" + (i + 1)]: data });
    }
